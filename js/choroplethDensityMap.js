...
Promise.all([
    ...
    d3.csv("data/land-area-km.csv"), // CSV file for land area
    ...
    const emissionsMap = new Map(
        emissionsData
            .filter(
                (d) => +d.Year === emissionsYear && d.Code && !isNaN(+d["Annual CO₂ emissions"])
            )
    ...
    // Define the color scale (using a logarithmic scale for better distribution)
    const maxDensity = d3.max(Array.from(densityData.values()));
    const colorScale = d3
        .scaleLog()
        .domain([1, maxDensity]) // Avoid zero by starting from 1
    ...
    svgDensityMap
        ...
        .on("mouseover", (event, d) => {
            ...
            d3.select("#tooltip")
                ...
                .text(
                    `${d.properties.NAME}: ${density ? density.toFixed(2) : "Data not available"
                    } tons/km²`
                );
        })
        .on("mousemove", (event) => {
